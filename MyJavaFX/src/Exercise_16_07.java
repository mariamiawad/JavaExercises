...
public class Exercise_16_07 extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Label hourlLabel = new Label("Hours");
		...
		hBox.getChildren().addAll(hourlLabel, hourField, minuteLabel, minuteField, secondLabel, secondField);
		BorderPane pane = new BorderPane();
		pane.setCenter(clockPane);
		pane.setBottom(hBox);
		Scene scene = new Scene(pane, 300, 300);
		primaryStage.setTitle("Exercise_16_07");
		primaryStage.setScene(scene);
		primaryStage.show();
		hourField.setOnAction(e->{
			int hour = Integer.parseInt(hourField.getText());
			clockPane.setHour(hour);
			
		});
		minuteField.setOnAction(e->{
			int minute = Integer.parseInt(minuteField.getText());
			clockPane.setMinute(minute);
		});
		secondField.setOnAction(e->{
